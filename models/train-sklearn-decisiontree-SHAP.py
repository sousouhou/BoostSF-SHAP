...
generateModelFolder  = "decisiontree-model01"  # the folder to store result
...
DT_params={     # parameters for DecisionTree
    'max_depth'  :  10,
    'random_state' : 0
}

model = tree.DecisionTreeRegressor(** DT_params)
# fit model 
model.fit(Xtrain, ytrain)

# save model
joblib.dump(model, "%s/trained-model.pkl"%generateModelFolder)
...
RMSE = root_mean_squared_error(ytrain_predicted, ytrain)

reg = LinearRegression().fit(ytrain_predicted.reshape(-1,1),  ytrain)  # ytrain_predicted, 1D->2D
ytrain_LR = reg.predict(ytrain_predicted.reshape(-1,1))

sum01 = 0.0
N = ytrain.shape[0]
for i in range(0, N):
    sum01 += (ytrain[i] - ytrain_LR[i])**2
SD = math.sqrt( sum01/(N-1.0) )
...
if flagConductTest == True :
    ...
    RMSE = root_mean_squared_error(ytest_predicted, ytest)

    reg = LinearRegression().fit(ytest_predicted.reshape(-1,1), ytest)
    ytest_LR = reg.predict(ytest_predicted.reshape(-1,1))

    sum02 = 0.0
    N = ytest.shape[0]
    for i in range(0, N):
        sum02 += (ytest[i] - ytest_LR[i])**2
    SD = math.sqrt( sum02/(N-1.0) )
